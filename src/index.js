...
client.on('voiceStateUpdate', (oldMember, newMember) => {
	...
	if (newMember.channelID) {
		db.VoiceChannel.findOne({
			...
			.then(data => {
				if (data) {
					console.log(`[log]: data found `, data.dataValues)
					const unit = vcGroup[data.category].unit
					newMember.guild.channels.create(`${newMember.member.displayName}\'s ${unit}`, {
							type: 'voice',
